...
def sherlock(username, site_data, verbose=False, tor=False, unique_tor=False,
	...
	for social_network, net_info in site_data.items():
		...
		if error_type == "message":
			...
			soup=BeautifulSoup(rp.get(r.url).content,'html.parser')
			if not 'Sorry, that page does' in soup.text:
...
def main():
	...
	action="store_true", dest="print_found_only", default=True,
	help="Do not output sites where the username was not found."
	)
	parser.add_argument("--no-color",
	action="store_true", dest="no_color", default=True,
	...
	while 1:
		...
		if uuu=="":
			break
		usernames.append(uuu)
	args.username = usernames
	args.folderoutput='Searched'
	args.rank=True
	console.clear()
	...
	for username in args.username:
		...
		if args.output:
			...
			file = open(os.path.join('Searched',
